            ...
            if (Model.Security.IsView(ModuleNameList.Condition_Inspection))
            {
                dashboardmenuitems.Add(new DashboardMenuItem()
                {
                    ...
                    MenuItems = new ObservableCollection<MenuItem>()
                    {
                        ...
                        new MenuItem() { Name = "Gantry Sign Visual Inspection (Form G1/G2)" },
                        new MenuItem() { Name = "Retaining Wall Visual Inspection Record (Form R1/R2)" },
                        new MenuItem() { Name = "Temporary Traffic Management Audit (Form M)" },
                       // new MenuItem() { Name = "Bridge Summary (Form F5)" },
                       //new MenuItem() { Name = "Traffic Volume Count Form (Form T)" },
